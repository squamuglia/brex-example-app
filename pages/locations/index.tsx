...
import type { PaginatedResponse, Locations } from "@/lib/sharedTypes";

const LocationsPage: NextPage<{ data: PaginatedResponse<Locations> }> = ({
  data,
}) => {
  const router = useRouter();

  const handleEditButtonClick = React.useCallback(
    (id: string) => () => router.push(`/locations/${id}`),
    []
  );
    ...
    <div>
      ...
      <ul className="grid grid-cols-4 gap-2">
        {data.items.map((location) => (
          ...
          >
            {/* <button
                onClick={handleEditButtonClick(location.id)}
